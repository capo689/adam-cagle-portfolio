...
var track=$('track');function scrub(ev){var rect=track.getBoundingClientRect();clock=Math.max(0,Math.min(1,(ev.clientX-rect.left)/rect.width))*duration;var state=locate(clock);currentNode='';if(state.on){record.visible=true;recordHalo.visible=true;record.position.copy(state.pos);recordHalo.position.copy(state.pos);renderGuide(state.activeId);if(follow)followTarget(state.pos,true);}activePath(state.activeEdge);completedPath();$('fill').style.width=(clock/duration*100)+'%';$('head').style.left=(clock/duration*100)+'%';$('now').textContent=clock>=duration-.01?'Run complete':cfg.stages[stageFor(state.activeId)];applyCamera();renderer.render(scene,camera);}
