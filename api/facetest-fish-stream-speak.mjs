...
export default async function handler(req, res) {
  ...
  if (!guardFacetestRequest(req, res, {limit: 60})) return;
  ...
  try {
    upstream = await fetch(OPENROUTER_SPEECH_URL, {
      ...
      headers: {
        ...
        "HTTP-Referer": "https://adamcagle.com/FACETEST/fish-stream-lab.html",
        "X-Title": "FACETEST Fish Streaming Lab"
      },
      body: JSON.stringify({
        model: MODEL,
  ...
  if (!upstream.ok) {
    ...
    console.error("FACETEST Fish streaming error", upstream.status, detail.slice(0, 1000));
  ...
  if (!upstream.body) return reject(res, 502, "Fish Audio returned no stream");

  res.statusCode = 200;
  res.setHeader("Content-Type", upstream.headers.get("content-type") || "audio/pcm;rate=44100");
  res.setHeader("Cache-Control", "no-store, no-transform");
  res.setHeader("X-Content-Type-Options", "nosniff");
  res.setHeader("X-FACETEST-PCM-Rate", upstream.headers.get("x-audio-sample-rate") || "44100");
  ...
  try {
    ...
    console.error("FACETEST Fish stream relay error", error instanceof Error ? error.message : error);
    res.end();
  }
}
