...
var ROLE = {
  det:   { c: 0x383838, tag: 'DETERMINISTIC' },
  gate:  { c: 0xcc0020, tag: 'POLICY GATE' },
  model: { c: 0x526b8c, tag: 'MODEL-ASSISTED' },
  human: { c: 0x1a1a1a, tag: 'HUMAN AUTHORITY' }
};
...
scene.fog = new THREE.Fog(0xf4f4f4, 42, 142);

var camera = new THREE.PerspectiveCamera(52, viewport.w / viewport.h, 0.1, 900);

scene.add(new THREE.AmbientLight(0xffffff, 0.92));
var key = new THREE.DirectionalLight(0xffffff, 0.74); key.position.set(4, 8, 10); scene.add(key);
var rim = new THREE.DirectionalLight(0xcc0020, 0.12); rim.position.set(-8, -3, -6); scene.add(rim);
...
function cardTexture(name, role) {
  ...
  c.fillStyle = role === 'model' ? '#d7e5f7' : (role === 'human' ? '#1a1a1a' : '#ffffff'); c.fill();
  c.lineWidth = 3; c.strokeStyle = role === 'model' ? '#526b8c' : (role === 'human' ? '#1a1a1a' : '#dadce0'); c.stroke();
  ...
  c.fillStyle = role === 'human' ? '#ffffff' : '#1a1a1a';
...
scene.add(new THREE.LineSegments(lineGeo, new THREE.LineBasicMaterial({
  color: 0x8b8f96, transparent: true, opacity: 0.34
})));
var activeLine = new THREE.Line(new THREE.BufferGeometry(), new THREE.LineBasicMaterial({ color:0xcc0020, transparent:true, opacity:.96 }));
...
function recordCard(id) {
  ...
  var c = cv.getContext('2d');
  c.shadowColor = 'rgba(26,26,26,.2)'; c.shadowBlur = 18; c.shadowOffsetY = 9;
  c.fillStyle = '#ffffff'; c.fillRect(26, 20, 308, 388);
  c.shadowColor = 'transparent';
  c.strokeStyle = '#cc0020'; c.lineWidth = 8; c.strokeRect(26, 20, 308, 388);
  c.fillStyle = '#cc0020'; c.fillRect(26, 20, 308, 62);
  ...
  c.fillStyle = '#1a1a1a'; c.font = '900 66px ui-monospace,SFMono-Regular,Menlo,monospace'; c.fillText(id, 48, 142);
  c.fillStyle = '#62666d'; c.font = '800 18px ui-monospace,SFMono-Regular,Menlo,monospace'; c.fillText('MIGRATION REVIEW', 48, 193);
  c.fillStyle = '#dadce0';
  c.fillRect(48, 232, 238, 13); c.fillRect(48, 264, 190, 13); c.fillRect(48, 296, 222, 13);
  c.fillStyle = '#cc0020'; c.beginPath(); c.arc(286, 354, 22, 0, Math.PI * 2); c.fill();
...
function buildRunner(rec) {
  ...
  var col = new THREE.Color(0xcc0020);
...
function stepTo(delta) {
  ...
  var n = byName[targetName]; if (n) { n.hl = 1; n.hlColor.set(0xcc0020); }
}
...
$('change-scenario').addEventListener('click', function () { pause(); $('launcher').className = 'launcher'; });
