...
import { aiSkills, aiWork, supportingAiCards, type AiWorkItem } from "@/WINNER/content/ai-content";
import { GuidedWorkflowViewer } from "@/WINNER/components/guided-workflow-viewer";
import { ElectricShimmerTitle } from "@/WINNER/components/electric-shimmer-title";
import { AiSystemPortrait } from "@/WINNER/components/ai-system-portrait";
import { GoldPixelWord } from "@/WINNER/components/gold-pixel-word";
import { showAceNarration } from "@/WINNER/lib/ace-transcript";
import { useModalAccessibility } from "@/WINNER/lib/modal-accessibility";
...
export function AiPage({ voiceEnabled, routePrefix = "" }: { voiceEnabled: boolean; routePrefix?: string }) {
  ...
  function openItem(item: AiWorkItem) {
    ...
    setSelected(item);
    setPlayback("idle");
    const path = `${routePrefix}/ai/${item.id}`;
  ...
  function closeItem() {
    ...
    setSelected(null);
    setPlayback("idle");
    if (window.location.pathname.startsWith(`${routePrefix}/ai/`)) window.history.replaceState({}, "", `${routePrefix}/ai`);
  }
  ...
  async function openGuidedWorkflow(item: AiWorkItem) {
    ...
    setPlayback("idle");
    setGuidedItem(item);
    const path = `${routePrefix}/ai/${item.id}/workflow`;
    if (window.location.pathname !== path) window.history.pushState({}, "", path);
  }
  ...
  return (
    <div className="ai-page">
      ...
      {guidedItem && (
        <GuidedWorkflowViewer item={guidedItem} voiceEnabled={voiceEnabled} onClose={() => { setGuidedItem(null); window.history.replaceState({}, "", `${routePrefix}/ai`); }} />
      )}
    </div>
  );
}
