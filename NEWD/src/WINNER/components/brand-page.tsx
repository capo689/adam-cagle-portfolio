...
import { figueroaBookPages } from "@/WINNER/content/figueroa-book";
...
export function BrandPage({ voiceEnabled, routePrefix = "" }: { voiceEnabled: boolean; routePrefix?: string }) {
  ...
  useEffect(() => {
    ...
    const openFeature = (event: Event) => {
      ...
      if (target === "agency") {
        if (window.location.pathname !== `${routePrefix}/brand/agency`) window.history.pushState({}, "", `${routePrefix}/brand/agency`);
        scrollToTarget(".brand-agency-feature");
      }
      if (target === "figueroa") {
        if (window.location.pathname !== `${routePrefix}/brand/figueroa`) window.history.pushState({}, "", `${routePrefix}/brand/figueroa`);
