...
async function transcribe(blob: Blob) {
  const response = await fetch("/api/face2-transcribe", {
    method: "POST",
...
function queueSpeech(text: string) {
  ...
  if (!spoken) return;
  speechQueue = speechQueue.then(() => window.FACE2?.speak(spoken));
}
...
async function askGroq(userText: string) {
  ...
  const response = await fetch("/api/face2-chat", {
    method: "POST",
...
window.addEventListener("face2:ready", () => {
  mic.hidden = false;
  setStatus("Groq · ready");
});

if (window.FACE2) window.FACE2.ask = ask;
