...
var track=$('track');function scrub(ev){var rect=track.getBoundingClientRect();clock=Math.max(0,Math.min(1,(ev.clientX-rect.left)/rect.width))*duration;currentNode='';}
...
renderer.domElement.addEventListener('pointerdown',function(ev){renderer.domElement.setPointerCapture(ev.pointerId);drag={x:ev.clientX,y:ev.clientY,moved:false};});
renderer.domElement.addEventListener('pointermove',function(ev){if(!drag)return;var dx=ev.clientX-drag.x,dy=ev.clientY-drag.y;if(Math.abs(dx)+Math.abs(dy)>3){drag.moved=true;follow=false;$('follow').className='btn';cam.targetYaw-=dx*.004;cam.targetPitch=Math.max(-.8,Math.min(.9,cam.targetPitch+dy*.003));}drag.x=ev.clientX;drag.y=ev.clientY;});
...
window.addEventListener('keydown',function(ev){if(ev.key===' '){ev.preventDefault();toggle();}else if(ev.key.toLowerCase()==='f'){follow=!follow;$('follow').className=follow?'btn on':'btn';}else if(ev.key.toLowerCase()==='r'){reset(true);}else if(ev.key==='Escape'){$('inspect').className='inspect';}});
