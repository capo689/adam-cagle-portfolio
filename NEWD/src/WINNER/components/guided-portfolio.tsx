...
import { FaceStage } from "@/WINNER/components/face-stage";
import { EntryParticleField } from "@/WINNER/components/entry-particle-field";
import { AiPage } from "@/WINNER/components/ai-page";
import { CopywritingPage } from "@/WINNER/components/copywriting-page";
import { BrandPage } from "@/WINNER/components/brand-page";
import { BrandClientWall } from "@/WINNER/components/brand-client-wall";
import { FunPage } from "@/WINNER/components/fun-page";
import { ElectricShimmerTitle } from "@/WINNER/components/electric-shimmer-title";
import { ProfileModals } from "@/WINNER/components/profile-modals";
import { showAceNarration } from "@/WINNER/lib/ace-transcript";
...
function winnerPath(pathname = "/") {
  return pathname === "/" ? "/WINNER" : `/WINNER${pathname}`;
}

function routeFromPath(pathname: string) {
  const winnerPathname = pathname === "/WINNER" ? "/" : pathname.replace(/^\/WINNER(?=\/)/, "");
  const [first = "", second, third] = winnerPathname.split("/").filter(Boolean);
...
export function GuidedPortfolio() {
  ...
  useEffect(() => {
    ...
    voiceModules.current = (async () => {
      await import("@/WINNER/lib/facetest-voice-stream");
      await import("@/WINNER/lib/facetest-conversation-handsfree");
      await window.FACETEST?.initialize();
    })();

    const searchParams = new URLSearchParams(window.location.search);
    const initialRoute = routeFromPath(window.location.pathname);
    const hasDeepRoute = window.location.pathname !== "/WINNER" && window.location.pathname !== "/WINNER/";
    if (hasDeepRoute) {
      void (async () => {
  ...
  useEffect(() => {
    const openProfile = (event: Event) => {
      ...
      if (profile === "resume") {
        ...
        if (window.location.pathname !== winnerPath("/resume")) window.history.pushState({}, "", winnerPath("/resume"));
      }
    };
  ...
  useEffect(() => {
    const navigate = (event: Event) => {
      ...
      if (window.location.pathname !== winnerPath(sectionPaths[destination as Section])) {
        window.history.pushState({}, "", winnerPath(sectionPaths[destination as Section]));
      }
  ...
  function openSection(destination: Section) {
    ...
    if (window.location.pathname !== winnerPath(sectionPaths[destination])) {
      window.history.pushState({}, "", winnerPath(sectionPaths[destination]));
    }
    window.requestAnimationFrame(scrollContentToTop);
  }
    ...
    <main className="experience" data-phase={phase} data-section={section}>
      ...
      <div className="site-shell" aria-hidden={phase !== "site"}>
          ...
          <section className="content-window" id="top">
            {section === "AI" ? (
              <AiPage routePrefix="/WINNER" voiceEnabled />
            ) : section === "Copywriting" ? (
              <CopywritingPage routePrefix="/WINNER" voiceEnabled />
            ) : section === "Brand" ? (
              <BrandPage routePrefix="/WINNER" voiceEnabled />
            ) : section === "Fun" ? (
              <FunPage />
            ) : (
              <>
                ...
                <div className="profile-card-grid" aria-label="About Adam Cagle">
                  <article className="profile-card resume-card">
                    ...
                    <button onClick={() => { setProfileOpen("resume"); window.history.pushState({}, "", winnerPath("/resume")); }} type="button" aria-label="Open Adam Cagle's resume">
          ...
          <ProfileModals open={profileOpen} onClose={() => { setProfileOpen(null); window.history.replaceState({}, "", winnerPath()); }} />
        </div>
    </main>
  );
}
