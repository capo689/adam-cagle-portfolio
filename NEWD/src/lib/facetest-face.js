...
const naturalEyeMaterial = new THREE.ShaderMaterial({
  uniforms: eyeUniforms,
  transparent: true,
  depthWrite: false,
  blending: THREE.NormalBlending,
...
const scleraMaterial = new THREE.ShaderMaterial({
  uniforms: eyeUniforms,
  transparent: true,
  depthWrite: false,
  blending: THREE.NormalBlending,
...
const LOOK_PRESETS = Object.freeze({
  original: Object.freeze({humanity:0,mosaic:0,naturalEyes:0,blueEyes:0,platinum:0,sclera:0,complexion:.45}),
  "original-blue": Object.freeze({humanity:0,mosaic:0,naturalEyes:0,blueEyes:1,platinum:0,sclera:0,complexion:.45}),
  "platinum-blue": Object.freeze({humanity:0,mosaic:0,naturalEyes:0,blueEyes:1,platinum:1,sclera:0,complexion:.45}),
  "human-gold": Object.freeze({humanity:1,mosaic:.035,naturalEyes:1,blueEyes:0,platinum:0,sclera:.82,complexion:.25}),
  "warm-mosaic": Object.freeze({humanity:1,mosaic:.045,naturalEyes:1,blueEyes:0,platinum:0,sclera:.60,complexion:.08}),
  "cool-bronze": Object.freeze({humanity:1,mosaic:.025,naturalEyes:1,blueEyes:0,platinum:0,sclera:.56,complexion:.92})
});
const requestedLook = new URLSearchParams(location.search).get("look");
let activeLook = Object.hasOwn(LOOK_PRESETS,requestedLook) ? requestedLook : "original";
let lookTarget = {...LOOK_PRESETS[activeLook]};
let lookCurrent = {...LOOK_PRESETS.original};
...
function createFaceSamples(data, count = 110000) {
  const cumulative = [];
  let total = 0;
...
function buildScene(data) {
  ...
  const scleraGeometry=attributeSet(2200,1.25);
  scleraCloud=new THREE.Points(scleraGeometry,scleraMaterial);
  scleraCloud.renderOrder=3;
  scene.add(scleraCloud);

  const eyeGeometry=attributeSet(760,1.55);
  eyesCloud=new THREE.Points(eyeGeometry,eyeMaterial);
  eyesCloud.renderOrder=4;
  scene.add(eyesCloud);

  const naturalEyeGeometry=attributeSet(760,1.32);
  naturalEyesCloud=new THREE.Points(naturalEyeGeometry,naturalEyeMaterial);
  naturalEyesCloud.renderOrder=5;
  scene.add(naturalEyesCloud);
  createField();
}
...
function setLook(name) {
  if (!Object.hasOwn(LOOK_PRESETS,name)) return false;
  activeLook=name;
  lookTarget={...LOOK_PRESETS[name]};
  window.dispatchEvent(new CustomEvent("facetest:lookchange",{detail:{name}}));
  return true;
}

function updateLook(delta) {
  for (const key of Object.keys(lookCurrent)) lookCurrent[key]=damp(lookCurrent[key],lookTarget[key],4.8,delta);
  faceUniforms.uHumanity.value=lookCurrent.humanity;
  faceUniforms.uMosaic.value=lookCurrent.mosaic;
  faceUniforms.uComplexion.value=lookCurrent.complexion;
  faceUniforms.uPlatinum.value=lookCurrent.platinum;
  fieldUniforms.uPlatinum.value=lookCurrent.platinum;
  eyeUniforms.uNatural.value=lookCurrent.naturalEyes;
  eyeUniforms.uBlueEyes.value=lookCurrent.blueEyes;
  eyeUniforms.uScleraOpacity.value=lookCurrent.sclera;
}
...
function deformVertices(c) {
  const humanity=THREE.MathUtils.clamp(lookCurrent.humanity,0,1);
  const reference=meshData.referenceRest||meshData.rest;
  for(let i=0;i<deformed.length;i++)deformed[i]=THREE.MathUtils.lerp(meshData.rest[i],reference[i],humanity);
  const leftEyeCenter=averageIndices(deformed,LEFT_EYE);
  const rightEyeCenter=averageIndices(deformed,RIGHT_EYE);
  for(let i=0;i<meshData.count;i++){
    const j=i*3;let x=deformed[j],y=deformed[j+1],z=deformed[j+2];
    if(humanity>.001){
      const noseVertical=smoothstep(-1.15,-.55,y)*(1-smoothstep(1.08,1.62,y));
      const noseHorizontal=1-smoothstep(.52,1.18,Math.abs(x));
      x*=1-humanity*noseVertical*noseHorizontal*.30;
    }
    if(LIPS.has(i)){
      const lower=y<-1.50;
...
function updateEyes(c,time) {
  ...
  for(let i=0;i<1600;i++){
    ...
    const socketHeight=.145*(1-sideBlink*.90);
    const j=i*3;
    scleraPos[j]=side[0]+Math.cos(a)*.52*rn;
    scleraPos[j+1]=side[1]+Math.sin(a)*socketHeight*rn;
    scleraPos[j+2]=side[2]+.082+(.012*(1-rn));
    scleraAlpha[i]=Math.pow(1-sideBlink,2.2)*smoothstep(1.0,.72,rn);
  }
  for(let i=0;i<760;i++){
    const side=i<380?left:right,local=i%380,q=(local+.5)/380,a=local*2.399963+time*.06,rn=Math.sqrt(q),r=THREE.MathUtils.lerp(.145,.155,lookCurrent.naturalEyes)*rn;
    const isLeft=i<380;
    const sideBlink=THREE.MathUtils.clamp(c.blink+(isLeft?c.blinkLeft:c.blinkRight)+c.eyeSquint*.25+(isLeft?c.eyeSquintLeft:c.eyeSquintRight)*.45-c.eyeWide*.18,0,1);
    ...
    naturalPos[j]=pos[j];naturalPos[j+1]=pos[j+1];naturalPos[j+2]=pos[j+2]+.006;
    ...
    const pupil=rn<pupilRadius?THREE.MathUtils.lerp(.035,1,lookCurrent.naturalEyes):1;
    alpha[i]=Math.pow(1-sideBlink,2.4)*smoothstep(1.0,.78,rn)*pupil;
    const naturalPupil=.30+THREE.MathUtils.clamp(c.pupil,0,1)*.05;
    const catchlight=rn>.38&&rn<.58&&Math.cos(a)<-.68&&Math.sin(a)>.62;
    naturalLight[i]=catchlight?.995:rn<naturalPupil?.01:rn<.84?(.55+.21*Math.sin(a*7.0+time*.18)):.09;
    naturalAlpha[i]=Math.pow(1-sideBlink,2.4)*smoothstep(1.0,.80,rn);
  }
  scleraCloud.visible=c.emerge>.62&&lookCurrent.sclera>.01;
  scleraCloud.geometry.attributes.position.needsUpdate=true;scleraCloud.geometry.attributes.aAlpha.needsUpdate=true;
  eyesCloud.visible=c.emerge>.62&&c.blink<.98&&lookCurrent.naturalEyes<.995;
  ...
  naturalEyesCloud.visible=c.emerge>.62&&c.blink<.98&&lookCurrent.naturalEyes>.005;
  naturalEyesCloud.geometry.attributes.position.needsUpdate=true;naturalEyesCloud.geometry.attributes.aLight.needsUpdate=true;naturalEyesCloud.geometry.attributes.aAlpha.needsUpdate=true;
}
...
// NEWD keeps ACE presentation-stable. Cursor movement does not steer the face
...
window.FACE={
  ...
  setLook,
  getLook(){return activeLook;},
  getLooks(){return Object.keys(LOOK_PRESETS);},
...
async function init() {
  const [response,referenceResponse]=await Promise.all([
    fetch("/face-assets/canonical_face_model.obj"),
    fetch("/face-assets/reference_male_face.json")
  ]);
  ...
  if(referenceResponse.ok){
    ...
    if(Array.isArray(reference.positions)&&reference.positions.length===data.rest.length)data.referenceRest=new Float32Array(reference.positions);
  }
  buildScene(data);
}
