...
import { copyCases, type CopyCase } from "@/WINNER/content/copywriting-content";
...
export function CopywritingPage({ voiceEnabled, routePrefix = "" }: { voiceEnabled: boolean; routePrefix?: string }) {
  ...
  function openDetail(item: CopyCase) {
    ...
    setSelected(item);
    setPlayback("idle");
    const path = `${routePrefix}/copy/${item.id}`;
  ...
  function closeDetail() {
    ...
    setSelected(null);
    setPlayback("idle");
    if (window.location.pathname.startsWith(`${routePrefix}/copy/`)) window.history.replaceState({}, "", `${routePrefix}/copy`);
  }
