...
const camera = new THREE.PerspectiveCamera(39, innerWidth / innerHeight, 0.1, 50);
...
function createField() {
  ...
  const target = innerWidth < 720 ? 76000 : 145000;
...
addEventListener("pointermove",event=>{pointerTarget.x=(event.clientX/innerWidth-.5)*2;pointerTarget.y=-(event.clientY/innerHeight-.5)*2;},{passive:true});

let resizeTimer;
addEventListener("resize",()=>{
  renderer.setPixelRatio(Math.min(devicePixelRatio,2));renderer.setSize(innerWidth,innerHeight);camera.aspect=innerWidth/innerHeight;camera.updateProjectionMatrix();
...
async function init() {
  const response=await fetch(new URL("assets/canonical_face_model.obj",import.meta.url));
