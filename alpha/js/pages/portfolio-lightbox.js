...
(function () {
  ...
  function step(direction) {
    ...
    // Direction-driven 3D card flip
    const isNext   = direction > 0;
    const exitX    = isNext ? -window.innerWidth * 0.32 :  window.innerWidth * 0.32;
    const exitRotY = isNext ? -16 :  16;   // outgoing flips away
    const enterX   = isNext ?  window.innerWidth * 0.32 : -window.innerWidth * 0.32;
    const enterRotY = isNext ?  16 : -16;  // incoming starts flipped, settles flat
    ...
    // Phase 1: outgoing slides + flips out + motion blur
    gsap.to(lbImg, {
      x: exitX,
      rotationY: exitRotY,
      opacity: 0,
      ...
      onComplete: () => {
        ...
        setText(newIdx);
        imageReady(lbImg).then(() => {
          // Phase 2: incoming starts on the opposite side, pre-rotated
          gsap.set(lbImg, {
            x: enterX,
            rotationY: enterRotY,
            opacity: 0,
    ...
    // Caption swap synced with the flip
